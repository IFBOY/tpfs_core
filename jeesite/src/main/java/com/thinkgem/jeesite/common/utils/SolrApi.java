...
public class SolrApi {
	...
	public void deleteDoc(String... ids) throws SolrServerException,
			IOException {
		for (String id : ids) {
	...
	public SolrDocument getDocById(String id) throws SolrServerException,
			IOException {
		return client.getById(id);
	}

	public List<ContentBean> queryAll(int startIndex, int pageSize)
		...
		if (list != null && list.size() > 0) {
			for (SolrDocument d : list) {
				...
				bean.setTitle(d.getFieldValue("create_date") == null ? "" : d
						.getFieldValue("create_date").toString());
				result.add(bean);
			}
		}
		return result;
	...
	public Map<String, Object> queryByParam(String condition, int startIndex,
			int PageSize) throws SolrServerException, IOException {
		Map<String, Object> result = new HashMap<String, Object>();
		result.put("rows", 0L);
		SolrQuery params = new SolrQuery("title:" + condition + " OR "
				+ "description:" + condition + " OR " + "keywords:" + condition
	...
	public List<Map<String, Long>> getKeyWordsCount()
			throws SolrServerException, IOException {
		...
		params.setFacet(true).setQuery("*:*").setFacetMinCount(1)
				.setFacetLimit(5).addFacetField("title")
				.addFacetField("keywords").addFacetField("content")
				.addFacetField("description");// 分片字段
